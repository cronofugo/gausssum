...
#######################################################
    ...
    # Write UVData.txt containing info on the transitions
        ...
        endwaveno = convertor(start,"nm","wavenumber")
        startwaveno = convertor(end,"nm","wavenumber")
        ...
        for x in range(numpts):
            ...
            outputfile.write(str(realx)+"\t"+str(convertor(realx,"wavenumber","nm"))+"\t"+str(t.spectrum[0,x]))
            if x<len(logfile.etenergies): # Write the oscillator strengths out also
                outputfile.write("\t\t\t"+str(logfile.etenergies[x])+"\t"+str(convertor(logfile.etenergies[x],"wavenumber","nm"))+"\t"+str(logfile.etoscs[x]))
            outputfile.write("\n")
        outputfile.close()
        ...
        if root:
            if max(t.spectrum[0,:])<1E-8: # Don't draw it if the spectrum is flat
                ...
                g = MPLPlot()
                xvalues_nm = [convertor(x,"wavenumber","nm") for x in t.xvalues]
                g.setlabels("Wavelength (nm)", r"$\epsilon$")
                g.data(zip(xvalues_nm,t.spectrum[0,:]))
                energies_nm = [convertor(x,"wavenumber","nm") for x in logfile.etenergies]
...
#######################################################
        ...
        endwaveno = convertor(start,"nm","wavenumber")
        startwaveno = convertor(end,"nm","wavenumber")
        sigma = convertor(FWHM, "eV", "wavenumber")
        Delta = sigma / 2.
        ...
        for x in range(numpts):
            ...
            if x < len(logfile.etenergies): # Write the R values out also
                outputfile.write( "\t\t\t%f\t%f\t%f" % (convertor(logfile.etenergies[x], "wavenumber", "nm"),
